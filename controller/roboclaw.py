...
class Roboclaw:
	...
	def _readchecksumword(self):
		...
		if len(data)==2:
			crc = (ord(chr(data[0]))<<8) | ord(chr(data[1]))
			return (1,crc)	
		return (0,0)
	...
	def Open(self):
		try:
			self._port = serial.Serial(port=self.comport, baudrate=self.rate, timeout=5, interCharTimeout=self.timeout)
