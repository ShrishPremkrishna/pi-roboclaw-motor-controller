...
class Roboclaw:
	...
	def _sendcommand(self,address,command):
		...
		self._port.write((address).to_bytes(length=1, byteorder='little'))
		self.crc_update(command)
		self._port.write((command).to_bytes(length=1, byteorder='little'))
		return
	...
	def _writebyte(self,val):
		...
		self._port.write((val&0xFF).to_bytes(length=1, byteorder='little'))
